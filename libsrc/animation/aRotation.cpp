...
void quat::FromRotation(const mat3& rot)
{
	...
	float vw = rot[0][0] + rot[1][1] + rot[2][2] + 1;
	if (vw > EPSILON)
	{
		mQ[VW] = sqrt(vw) / 2;
	...
	{
		mQ[VW] = (rot[2][1] - rot[1][2]) / (2.0*sqrt(1 + rot[0][0] - rot[1][1] - rot[2][2]));
		mQ[VX] = (2.0 * sqrt(1 + rot[0][0] - rot[1][1] - rot[2][2])) / 4;
		mQ[VY] = (rot[0][1] + rot[1][0]) / (2.0 * sqrt(1 + rot[0][0] - rot[1][1] - rot[2][2]));
		mQ[VZ] = (rot[0][2] + rot[2][0]) / (2.0 * sqrt(1 + rot[0][0] - rot[1][1] - rot[2][2]));
	}
	else if (rot[1][1] > rot[2][2])
	{
		mQ[VW] = (rot[0][2] - rot[2][0]) / (2.0 * sqrt(1 + rot[1][1] - rot[0][0] - rot[2][2]));
		mQ[VX] = (rot[0][1] + rot[1][0]) / (2.0 * sqrt(1 + rot[1][1] - rot[0][0] - rot[2][2]));
		mQ[VY] = (2.0 * sqrt(1 + rot[1][1] - rot[0][0] - rot[2][2])) / 4;
		mQ[VZ] = (rot[1][2] + rot[2][1]) / (2.0 + sqrt(1 + rot[0][0] - rot[1][1] - rot[2][2]));
	}
	else
	{
		mQ[VW] = (rot[1][0] - rot[0][1]) / (2.0 * sqrt(1 - rot[1][1] - rot[0][0] + rot[2][2]));
		mQ[VX] = (rot[0][2] + rot[2][0]) / (2.0 * sqrt(1 - rot[1][1] - rot[0][0] + rot[2][2]));
		mQ[VY] = (rot[1][2] + rot[2][1]) / (2.0 * sqrt(1 - rot[1][1] - rot[0][0] + rot[2][2]));
		mQ[VZ] = (2.0 * sqrt(1 - rot[1][1] - rot[0][0] + rot[2][2])) / 4;
	}
	//cout << rot << endl;
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
	double Q = Dot(q0, q1);
	double theta = acos(Q);
	quat q = (sin(theta*(1 - u)) / sin(theta))*q0 + (sin(theta*u) / sin(theta))*q1;
...
{
	double tr = sqrt(Dot(a, a) + Dot(b, b));
	quat q = (a + b) / tr;
...
void quat::ToAxisAngle(vec3& axis, double& angleRad) const
{
	...
	//angleRad = 0.0;
	double theta = 2 * acos(mQ[VW]);
	angleRad = theta;
	axis[VX] = mQ[VX] / (sin(theta / 2));
	axis[VY] = mQ[VY] / (sin(theta / 2));
	axis[VZ] = mQ[VZ] / (sin(theta / 2));
...
void quat::FromAxisAngle(const vec3& axis, double angleRad)
{
	...
	mQ[VW] = cos(theta);
	double t = sin(theta);
	mQ[VX] = t * (axis[VX] / sqrt((axis[VX] * axis[VX] + axis[VY] * axis[VY] + axis[VZ] * axis[VZ])));
	mQ[VY] = t * (axis[VY] / sqrt((axis[VX] * axis[VX] + axis[VY] * axis[VY] + axis[VZ] * axis[VZ])));
	mQ[VZ] = t * (axis[VZ] / sqrt((axis[VX] * axis[VX] + axis[VY] * axis[VY] + axis[VZ] * axis[VZ])));
}
