...
int IKController::computeLimbIK(ATarget target, AIKchain& IKchain, const vec3 midJointAxis, ASkeleton* pIKSkeleton)
{
	...
	if ((endJointID >= 0) && (endJointID < pIKSkeleton->getNumJoints()))
	{
		...
		vec3 error = mTarget0.getGlobalTranslation() - m_pEndJoint->getGlobalTranslation();
		// 2. compute vector between end Joint and base joint
		vec3 ETB = m_pEndJoint->getGlobalTranslation() - m_pBaseJoint->getGlobalTranslation();
		// 3. compute vector between target and base joint
		vec3 TTB = mTarget0.getGlobalTranslation() - m_pBaseJoint->getGlobalTranslation();
		// 4. Compute desired angle for middle joint 
		vec3 MTB = m_pMiddleJoint->getGlobalTranslation() - m_pBaseJoint->getGlobalTranslation();
		vec3 ETM = m_pEndJoint->getGlobalTranslation() - m_pMiddleJoint->getGlobalTranslation();
		vec3 TTM = mTarget0.getGlobalTranslation() - m_pMiddleJoint->getGlobalTranslation();
		double MTBlength = MTB.Length();
		double ETMlength = ETM.Length();
		double TTBlength = TTB.Length();
		double ETBlength = ETB.Length();
		double TTMlength = TTM.Length();
		double cosin = ((MTBlength*MTBlength + ETMlength * ETMlength)-TTBlength * TTBlength) / (2 * MTBlength*ETMlength);
		if (cosin < -1)
		{
			cosin = -1;
		...
		double midangle =M_PI-(phi);

		// 5. given desired angle and midJointAxis, compute new local middle joint rotation matrix and update joint transform
		mat3 Newrot;
		Newrot.FromAxisAngle(midJointAxis, midangle);
		m_pMiddleJoint->setLocalRotation(Newrot);
		m_pMiddleJoint->updateTransform();
		// 6. compute vector between target and base joint
		ETB = m_pEndJoint->getGlobalTranslation() - m_pBaseJoint->getGlobalTranslation();
		TTB = mTarget0.getGlobalTranslation() - m_pBaseJoint->getGlobalTranslation();
		// 7. Compute base joint rotation axis (in global coords) and desired angle
		vec3 BaseAxis = ETB.Cross(TTB);
		double BaseAngle = acos((ETB*TTB)/(ETB.Length()*TTB.Length()));
		// 8. transform base joint rotation axis to local coordinates
		vec3 LocalRotAxis = ((m_pBaseJoint->getLocal2Global()).Inverse())*BaseAxis;
		// 9. given desired angle and local rotation axis, compute new local rotation matrix and update base joint transform
		mat3 BaseRot;
		BaseRot.FromAxisAngle(LocalRotAxis, BaseAngle);
		m_pBaseJoint->setLocalRotation(m_pBaseJoint->getLocalRotation()*BaseRot);
...
int IKController::computeCCDIK(ATarget target, AIKchain& IKchain, ASkeleton* pIKSkeleton)
{
	...
	if ((endJointID >= 0) && (endJointID < pIKSkeleton->getNumJoints()))
	{
		...
		{
			int count = 0;
			AJoint* Joint0 = m_pEndJoint->getParent();
			AJoint*	Root = pIKSkeleton->getRootNode();
			while(Joint0 != Root)
			{
				//TODO:
				vec3 CTE =desiredEndPos- Joint0->getGlobalTranslation();// CTE=Current Joint to End Joint
				// 1. compute axis and angle for each joint in the IK chain (distal to proximal) in global coordinates
				vec3 error = desiredEndPos - m_pEndJoint->getGlobalTranslation();
				vec3 Axis = CTE.Cross(error);
				double Angle = (Axis.Length()) / (CTE*CTE + error * CTE);
				// 2. once you have the desired axis and angle, convert axis to local joint coords 
				ATransform LocalRot = Joint0->getLocal2Global();
				vec3 AxisLocal = (LocalRot.Inverse())*Axis;
				// 3. multiply angle by corresponding joint weight value
				double Weight = IKchain.getWeight(count);
				Angle = Weight * Angle;
				// 4. compute new local joint rotation matrix
				mat3 New;
				New.FromAxisAngle(AxisLocal, Angle);
				mat3 Local = Joint0->getLocalRotation();
				Joint0->setLocalRotation(Local*New);

				// 5. update joint transform
				Joint0->updateTransform();
				count+=1;
				Joint0 = Joint0->getParent();
				// 6. repeat same operations above for each joint in the IKchain from end to base joint
			}
			numIterations+=1;
			vec3 error = desiredEndPos - m_pEndJoint->getGlobalTranslation();
			double errlength = error.Length();
			if (errlength < epsilon)
			{
				break;
			}
		}
