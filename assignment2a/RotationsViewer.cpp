...
mMode(DEMO1), mXAngle(50), mYAngle(0), mZAngle(0), mRotOrder(XYZ)
{
	...
	mCSpline.appendKey(vec3(1.0, 0.0, 1.5));
	mCSpline.appendKey(vec3(1.0, 1.0, 1.0));
	mCSpline.appendKey(vec3(1.0, 0.5, 0.5));
	mCSpline.appendKey(vec3(0.8, 0.6, 0.4));
	mCSpline.appendKey(vec3(-0.9, 0.4, 0.0));
	mCSpline.appendKey(vec3(0.2, 0.6, 0.4));
	mCSpline.appendKey(vec3(0.0, 0.4, 0.4));
	mCSpline.appendKey(vec3(-1.0, 0.7, -0.8));
	mCSpline.appendKey(vec3(1.0, -0.5, -0.9));
	mCSpline.setInterpolationType(ASplineVec3::CUBIC_BSPLINE);
	mCSpline.computeControlPoints();

	mPSpline.appendKey(vec3(1.0, 0.0, 1.0));
	mPSpline.appendKey(vec3(0.0, 1.0, 1.0));
	mPSpline.appendKey(vec3(0.0, 1.8, 2.5));
	mPSpline.appendKey(vec3(-3.0, -2.6, 0.4));
	mPSpline.appendKey(vec3(0.5, 0.9, 3.0));
	mPSpline.setInterpolationType(ASplineVec3::CUBIC_CASTELJAU);
	mPSpline.computeControlPoints();

	mSSpline.appendKey(vec3(1.0, 1.0, 1.5));
	mSSpline.appendKey(vec3(1.4, 0.2, 1.8));
	mSSpline.appendKey(vec3(-2.0,-3.0,2.0));
	mSSpline.appendKey(vec3(1.4, 1.3, 1.5));
	mSSpline.appendKey(vec3(1.6, 0.2, -0.7));
	mSSpline.appendKey(vec3(1.0, 0.2, 0.8));
	mSSpline.appendKey(vec3(2.0, 0.2, 0.0));
	mSSpline.appendKey(vec3(0.1, -0.3, 0.6));
	mPSpline.setInterpolationType(ASplineVec3::CUBIC_MATRIX);
	mPSpline.computeControlPoints();
}
...
void RotationsViewer::drawDemo3()
{
	double time = mClock->totalElapsedTime();
	quat q = mQuatSpline.getCachedValue(time);
	double angle;
	...
	q.ToAxisAngle(axis, angle);
	vec3 color, pos, size;
	double t = mPSpline.getNormalizedTime(time);
	color = mCSpline.getValue(t);
	pos = mPSpline.getValue(t);
	size = mSSpline.getValue(t);
...
void RotationsViewer::initializeGui()
{
	...
	TwEnumVal modeTypeEV[] = { { DEMO1, "Convert" }, { DEMO2, "Curve" },{ DEMO3, "Color"} };
